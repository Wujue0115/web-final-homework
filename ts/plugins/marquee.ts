type marqueeModes = "repeat" | "return";
...
function createMarqueeWrapper(element: HTMLElement, options: marqueeOptions) {
...
function setKeyframes(element: HTMLElement) {
...
function setRepeatEffect(marqueeFirst: HTMLElement, marqueeSecond: HTMLElement) {
...
function setReturnEffect(marqueeFirst: HTMLElement, options: marqueeOptions) {
  assignStyles(marqueeFirst, {
...
function addMarqueeEffect(element: HTMLElement, options: marqueeOptions = defaultMarqueeOptions) {
...
const defaultMarqueeOptions: marqueeOptions = {
  duration: "10s",
...
export function marqueeInit() {
  const elements = document.querySelectorAll<HTMLElement>("[w-marquee]");
  for (const element of elements) {
    const options: marqueeOptions = {
      ...
      mode: element.getAttribute("w-mode") as marqueeModes,
    }

    for (const key in options) {
      options[key] ||= defaultMarqueeOptions[key];
    }
